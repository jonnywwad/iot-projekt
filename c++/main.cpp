...
//hier alle vorhandenen sensoren definieren <========= hier individuell neue Sensoren hinzufügen!!! (rest bleibt auf jedem sensor-pi gleich)

int sensoranzahl = 2;
struct sensoreintrag sensoren[] = {
    {
name: "Temperatursensor-Zimmer",
      mqtt_topic: "sensoren/zimmer1/temperatur",      //MQTT topic (daten werden dort hingesendet)
      ...
      read_SensorFunktion:   readfloat_ds18b20       
          //!!!dementsprechende datei in der diese funktion definiert ist (von der gruppe) muss natuerlich oben included vgl samplesensor werden!!!
    },
    {
name: "Beispielsensor",
      mqtt_topic: "testing/beispielsensor",
      timestamp: 0,
      value: 0,
      read_SensorFunktion:  readfloat_samplesensor
    },
};
    ...
    for (; ;){
        ...
        for(int i = 0;i<sensoranzahl;i++){
            ...
            if(sensoren[i].value != -1){ //pruefen ob gueltigen wert erhalten (errorcode -1)
                ...
                std::string json_string = Val_float2JSON(sensoren[i].value);
